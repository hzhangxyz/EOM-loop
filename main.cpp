...
struct U_matrix {
  ...
  void generate_adding() {
    ...
    for (auto p2 = 0; p2 < n; p2++) {
      for (auto i1 = 0; i1 < n - 1; i1++) {
        auto o1 = i1 + 1;
        get_element(o1, p2, i1, p2, &a_1) =
            get_element(i1, p2, o1, p2, &a_1_dagger) = std::sqrt(o1);
      }
    }
    // a_2_dagger
    for (auto p1 = 0; p1 < n; p1++) {
      for (auto i2 = 0; i2 < n - 1; i2++) {
        auto o2 = i2 + 1;
        get_element(p1, o2, p1, i2, &a_2) =
            get_element(p1, i2, p1, o2, &a_2_dagger) = std::sqrt(o2);
      }
    }
#if 0
    std::cout << "a1dagger\n";
    ...
    for (auto i = 0; i < n * n * n * n; i++) {
      adding_1[i] = std::conj(tau1) * a_1[i] +
                    std::conj(sigma1) * a_1_dagger[i] +
                    std::conj(nu1) * a_2[i] + std::conj(mu1) * a_2_dagger[i];
    }
    // adding 2 = tau2* a1 + sigma2* a1dagger + nu2* a2 + mu2* a2dagger
    for (auto i = 0; i < n * n * n * n; i++) {
      adding_2[i] = std::conj(tau2) * a_1[i] +
                    std::conj(sigma2) * a_1_dagger[i] +
                    std::conj(nu2) * a_2[i] + std::conj(mu2) * a_2_dagger[i];
    }
  }
  ...
  std::complex<double> &get_element(long in1, long in2, long out1, long out2,
                                    std::vector<complex> *m = nullptr) {
    if (m == nullptr) {
      m = &matrix;
    }
    long offset = 0;
  ...
  void generate_all(int _n, double _r, double _omega, double _phi,
                    double _psi) {
    n = _n;
    create_matrix();
    r = _r;
...
complex trace_mps(int l, const TAT::Tensor<complex> &a) {
  auto as = a.shrink({{"1.I1", 0}, {"2.I1", 0}})
                .edge_rename({{"1.O2", "A.1.O2"}, {"2.O2", "A.2.O2"}});
  auto result = as.trace({{"1.O1", "2.O1"}});
  for (auto t = 0; t < l - 1; t++) {
    result =
        result
            .contract(a.edge_rename({{"1.O2", "A.1.O2"}, {"2.O2", "A.2.O2"}}),
                      {{"A.1.O2", "1.I1"}, {"A.2.O2", "2.I1"}})
            .trace({{"1.O1", "2.O1"}});
  }
  result =
      result
...
complex contract_mps(int l, const TAT::Tensor<complex> &a,
                     const TAT::Tensor<complex> &b) {
  auto as = a.shrink({{"1.I1", 0}, {"2.I1", 0}})
                .edge_rename({{"1.O2", "A.1.O2"}, {"2.O2", "A.2.O2"}});
  auto bs = b.shrink({{"1.I1", 0}, {"2.I1", 0}})
                .edge_rename({{"1.O2", "B.1.O2"}, {"2.O2", "B.2.O2"}});
  ...
  for (auto t = 0; t < l - 1; t++) {
    result =
        result
            .contract(a.edge_rename({{"1.O2", "A.1.O2"}, {"2.O2", "A.2.O2"}}),
                      {{"A.1.O2", "1.I1"}, {"A.2.O2", "2.I1"}})
            .contract(b.edge_rename({{"1.O2", "B.1.O2"}, {"2.O2", "B.2.O2"}}),
                      {{"B.1.O2", "1.I1"},
                       {"B.2.O2", "2.I1"},
                       {"1.O1", "1.O1"},
                       {"2.O1", "2.O1"}});
